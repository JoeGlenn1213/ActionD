	...
	s.AddTool(
		mcp.NewTool("actiond_handoff_pack",
			...
			mcp.WithString("task_id",
				mcp.Description("Task ID; when provided, the task report is also queried to fill in goal and decisions"),
			),
			mcp.WithString("project_id",
				mcp.Description("Project ID in the task management system (defaults to the repository name)"),
			),
			mcp.WithString("from_agent",
				mcp.Description("Identity of the agent handing off the work (e.g., 'codex')"),
			),
			mcp.WithString("to_agent",
				mcp.Description("Identity of the agent receiving the work (e.g., 'claude')"),
			),
			mcp.WithString("suggested_next_action",
				mcp.Description("Next step the receiving agent should take"),
			),
			mcp.WithString("goal",
				mcp.Description("One-sentence task goal"),
			),
			mcp.WithString("pending_work",
				mcp.Description("Comma-separated list of pending work items"),
			),
			mcp.WithNumber("ttl_hours",
				mcp.Description("How long the handoff stays valid, in hours (default 24)"),
			),
