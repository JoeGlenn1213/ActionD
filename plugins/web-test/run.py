#!/usr/bin/env python3
"""
web-test - run the repository test script (Jest / Vitest) for ActionD.

Detects whether a test script is declared, installs dependencies when
``node_modules`` is missing, runs the tests, and parses the Jest/Vitest summary
for the report.

Output contract: a single V1 ActionResult JSON on stdout (all logs go to
stderr). Fail-closed: a failing test run blocks with status=failed +
decision=deny and exit code 1; a repo with no test script is skipped (success).
"""
...
def parse_test_summary(output: str) -> Tuple[int, int, int]:
    """Best-effort parse of Jest/Vitest summary. Returns (passed, failed, total)."""
    text = output or ""
    ...
    for line in text.splitlines():
        if "Tests:" in line:
            seg = line.split("Tests:", 1)[1]
            for part in seg.split(","):
                part = part.strip()
                m = re.match(r"(\d+)\s+(passed|failed|total|skipped|todo)\b", part, re.IGNORECASE)
                if m:
                    n = int(m.group(1))
                    key = m.group(2).lower()
                    if key == "passed":
    ...
    if total or passed or failed:
        return passed, failed, total

    # Vitest: "Tests  1 failed | 2 passed (3)" or "Tests  5 passed (5)"
    for line in text.splitlines():
        s = line.strip()
        if s.startswith("Tests"):
            fm = re.search(r"(\d+)\s+failed", s, re.IGNORECASE)
            pm = re.search(r"(\d+)\s+passed", s, re.IGNORECASE)
            tm = re.search(r"\((\d+)\)", s)
            if fm:
...
def main() -> None:
    ...
    if not test_script or "no test specified" in str(test_script):
        artifacts = save_report(
            artifact_dir,
            "test-report.txt",
            ["SKIPPED: no meaningful test script declared in package.json"],
        )
        emit(
            to_action_result(
                {
                    "status": "success",
                    "summary": "skipped: no test script declared",
                    "artifacts": artifacts,
                    "signals": {"tests_passed": False, "tests_skipped": True, "tests_failed": 0},
                },
                PLUGIN_ID, CAPABILITY, 0, LANGUAGE,
            ),
            0,
    ...
    if not ok:
        artifacts = save_report(artifact_dir, "test-report.txt", logs)
        emit(
            to_action_result(
                {
                    "status": "failed",
                    ...
                    "signals": {"tests_passed": False, "tests_failed": 1},
                },
    ...
    if shutil.which(pm) is None:
        artifacts = save_report(artifact_dir, "test-report.txt", logs + [f"[error] '{pm}' not found on PATH"])
        emit(
            to_action_result(
                {
                    "status": "failed",
    ...
    try:
        ...
        logs.append(f"[error] failed to run test command: {exc}")
        artifacts = save_report(artifact_dir, "test-report.txt", logs)
        emit(
            to_action_result(
                {
                    "status": "failed",
                    "summary": f"failed to run test command: {exc}",
                    "artifacts": artifacts,
    ...
    passed, failed, total = parse_test_summary(combined)
    ...
    if proc.returncode != 0:
        ...
        summary = f"tests failed (exit {proc.returncode}): {head}"
        emit(
            to_action_result(
                {
                    "status": "failed",
                    "summary": summary,
                    "artifacts": artifacts,
                    "hints": ["tests failed; inspect test-report.txt for the full log"],
                    "signals": {"tests_passed": False, "tests_failed": failed, "tests_total": total, "tests_passed_count": passed},
                },
                PLUGIN_ID, CAPABILITY, failed, LANGUAGE,
            ),
            1,
        )

    if total:
        summary = f"tests passed ({passed} passed, {total} total)"
    elif passed:
        summary = f"tests passed ({passed} passed)"
    else:
        ...
        to_action_result(
            {
                ...
                "summary": summary,
                "artifacts": artifacts,
                "signals": {"tests_passed": True, "tests_failed": failed, "tests_total": total, "tests_passed_count": passed},
            },
            PLUGIN_ID, CAPABILITY, failed, LANGUAGE,
        ),
        0,
